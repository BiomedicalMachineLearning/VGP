...
def set_snpid_index(df):
    df["A1_first"] = (
        (df["A1"] < df["A2"]) | (df["A1"].str.len() > 1) | (df["A2"].str.len() > 1)
    )
    df["A1s"] = df["A2"].copy()
...
def ldscore_fromlist(flist, num=None):
    ...
    for fh_i, fh in enumerate(flist):
        ...
        if len(ldscore_array) > 0:
            if not series_eq(y.index, ldscore_array[0].index) or not series_eq(
                y.SNP, ldscore_array[0].SNP
            ):
                raise ValueError(
                    "LD Scores for concatenation must have identical SNP columns (and A1/A2 columns if such columns exist)."
                )
            else:  # keep SNP and CHR column from only the first file
                y = y.drop(columns=["SNP", "CHR"], axis=1)

        new_col_dict = {
            c: c + "_" + str(fh_i) for c in y.columns if c not in ["SNP", "CHR"]
        }
...
def annot_parser(fh, compression, frqfile_full=None, compression_frq=None, anno=None):
    ...
    df_annot.drop(
        columns=["SNP", "BP", "CM", "CHR", "A1", "A2"], inplace=True, errors="ignore"
    )
    df_annot = df_annot.astype(float)
    if anno is not None:
        df_annot = df_annot.loc[
            :, [c for c in df_annot.columns if (c == "SNP" or c in anno)]
        ]
...
def M(fh, num=None, N=2, common=False):
    ...
    if num is not None:
        x = np.sum(
            [parsefunc(sub_chr(fh, i) + suffix) for i in range(1, num + 1)], axis=0
        )
    else:
    ...
    if num is not None:  # 22 files, one for each chromosome
        ...
        for chr in tqdm(range(1, num + 1)):
            if frqfile is not None:
                df_annot_chr_list = [
                    annot_parser(
                        sub_chr(fh, chr) + annot_suffix[i],
                        annot_compression[i],
                        sub_chr(frqfile, chr) + frq_suffix,
                        frq_compression,
                        anno=anno,
                    )
                    for i, fh in enumerate(fh_list)
            ...
            if anno is not None:
                ...
                for a in anno:
                    assert (
                        a in list_c
                    ), "Annotation %s was not found in the annotations file" % (a)
        ...
        if frqfile is not None:
            ...
            df_annot_list = [
                annot_parser(fh + annot_suffix[i], annot_compression[i], anno=anno)
                for i, fh in enumerate(fh_list)
            ]
