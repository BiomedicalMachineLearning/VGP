...
class LD_Score_Regression(object):
    def __init__(
        ...
        if loco and step1_ii is not None:
            raise ValueError("LOCO cannot be used with a two-step estimator")
        if step1_ii is not None and self.constrain_intercept:
            ...
            if loco:
                ...
                self.jknife_ridge = jk.Jackknife_Ridge(
                    xc,
                    y,
                    n_blocks,
                    chr_num=chr_num,
...
class Hsq(LD_Score_Regression):
    ...
    def _overlap_output(
        self, category_names, overlap_matrix, M_annot, M_tot, print_coefficients
    ):
        ...
        prop_hsq_overlap = np.dot(overlap_matrix_prop, self.prop.T).reshape(
            (1, self.n_annot)
        )
        prop_hsq_overlap_var = np.diag(
            np.dot(np.dot(overlap_matrix_prop, self.prop_cov), overlap_matrix_prop.T)
        )
        prop_hsq_overlap_se = np.sqrt(np.maximum(0, prop_hsq_overlap_var)).reshape(
            (1, self.n_annot)
        )
        ...
        for i in range(self.n_annot):
            if one_d_convert(self.coef)[i] == 0 and one_d_convert(self.coef_se)[i] == 0:
                ...
                coef_z.append(
                    one_d_convert(self.coef)[i] / one_d_convert(self.coef_se)[i]
                )
...
class Gencov(LD_Score_Regression):
    ...
    def summary(self, ref_ld_colnames, P=None, K=None):
        ...
        out.append(
            "Total " + T + " scale gencov: " + s(self.tot) + " (" + s(self.tot_se) + ")"
        )
        if self.n_annot > 1:
