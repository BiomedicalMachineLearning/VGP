...
FINEMAPPER_SCRIPT = os.path.join(
    os.path.dirname(os.path.realpath(__file__)), "finemapper.py"
)
...
def create_finemapper_cmd(args, chr_num, start, end, url_prefix):
    ...
    cmd = "%s %s --chr %s --start %s --end %s --out %s" % (
        args.python3,
        FINEMAPPER_SCRIPT,
        chr_num,
        start,
...
def main(args):
    ...
    df_regions_keep = df_regions.apply(
        lambda r: np.sum(
    ...
    with open(args.jobs_file, "w") as f:
        for _, r in df_regions.iterrows():
            ...
            if args.pvalue_cutoff is not None:
                df_sumstats_r = df_sumstats.query(
                    "CHR==%d & %d <= BP <= %d" % (chr_num, start, end)
                )
                if np.all(df_sumstats_r["P"] > args.pvalue_cutoff):
                    continue
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--geno", default=None, help="Genotypes file (plink or bgen format)"
    )
    parser.add_argument(
        "--chr",
        default=None,
        type=int,
        help="Target chromosome (if not provided, all chromosomes will be considered)",
    )
    ...
    parser.add_argument(
        ...
        help="name of file of regions and their URLs",
    )
    parser.add_argument("--python3", default="python3", help="python3 executable")
    parser.add_argument(
        "--out-prefix", required=True, help="prefix of the output files"
    )
    parser.add_argument(
        "--jobs-file", required=True, help="name of file with fine-mapping commands"
    )
    parser.add_argument(
        "--pvalue-cutoff",
        type=float,
        default=None,
        help="only consider regions that have at least one SNP with a p-value greater than this cutoff",
    )
    ...
    if len(os.path.dirname(args.out_prefix)) > 0 and not os.path.exists(
        os.path.dirname(args.out_prefix)
    ):
        raise ValueError(
            "output directory %s doesn't exist" % (os.path.dirname(args.out_prefix))
        )
    if len(os.path.dirname(args.jobs_file)) > 0 and not os.path.exists(
        os.path.dirname(args.jobs_file)
    ):
        raise ValueError(
            "output directory %s doesn't exist" % (os.path.dirname(args.jobs_file))
        )
