...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--sumstats",
        required=True,
        help="Whitespace-delimited file with SNPs to extract. Must include columns CHR,BP,A1,A2",
    )
    parser.add_argument(
        "--out", required=True, help="Prefix of the name of the output file"
    )
    parser.add_argument(
        "--allow-missing",
        default=False,
        action="store_true",
        help="If specified, the script will not terminate if some SNPs are not found in the meta file",
    )
    parser.add_argument(
        "--q",
        type=int,
        default=100,
        help="The maximum ratio between the largest and smallest prior causal probabilities",
    )
    ...
    for c in df_snps.columns:
        if c not in SNP_COLUMNS and c in df_meta.columns:
            raise ValueError(
                "Column %s appears in both the sumstats files and in the meta-analysis files"
                % (c)
            )
    ...
    df = df_meta.merge(
        df_snps.drop(columns=SNP_COLUMNS, errors="ignore"),
        left_index=True,
        right_index=True,
    )
    ...
    if "Z" in df.columns:
        ...
        if is_flipped.sum() > 0:
            ...
            logging.info(
                "Flipping the Z-sign of %d SNPs that A1 in sumstats = A2 in the per-SNP h2 data"
                % (is_flipped.sum())
            )
    ...
    if df.shape[0] < df_snps.shape[0]:
        ...
        error_message = (
            "Not all SNPs in the SNPs file were found in the meta file. Wrote a list of missing SNPs to %s"
            % (args.out + ".miss.gz")
        )
