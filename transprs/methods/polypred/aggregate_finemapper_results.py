...
def main(args):
    ...
    if args.adjust_beta_freq:
        df_sumstats["BETA_MEAN"] /= np.sqrt(
            2 * df_sumstats["MAF"] * (1 - df_sumstats["MAF"])
        )
        df_sumstats["BETA_SD"] /= np.sqrt(
            2 * df_sumstats["MAF"] * (1 - df_sumstats["MAF"])
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--out", required=True, help="name of the aggregated output files"
    )
    parser.add_argument(
        "--allow-missing-jobs",
        default=False,
        action="store_true",
        help="whether to allow missing jobs",
    )
    parser.add_argument(
        "--regions-file",
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="If specified, the posterior estimates of the SNP effect sizes will be on per-allele scale rather than a per-standardized genotype scale",
    )
