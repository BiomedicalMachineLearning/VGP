...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--pips",
        required=True,
        help="Whitespace-delimited file with SNPs ids and PPs. Must include columns CHR,BP,A1,A2,PIP",
    )
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="If specified, the script will not terminate if some SNPs are not found in the annotation file",
    )
    parser.add_argument(
        "--pip-cutoff",
        type=float,
        default=0.95,
        help="Only extract annotations for SNPs with PIP greater than this cutoff (default is 0.95)",
    )
    ...
    if np.any(is_annot_missing):
        err_msg = (
            "The annotations of %d SNPs were not found in the annotation files"
            % (is_annot_missing.sum())
        )
        if args.allow_missing:
            ...
            raise ValueError(
                err_msg
                + ". To proceed without these SNPs, use the flag --allow-missing"
            )
