...
warnings.filterwarnings(
    "ignore",
    message="Coordinate descent with alpha=0 may lead to unexpected results and is discouraged.",
)
warnings.filterwarnings(
    "ignore",
    message="Objective did not converge. You might want to increase the number of iterations. Fitting data with very small alpha may cause precision problems.",
)
    ...
    def __init__(
            ...
            for i in tqdm(range(len(s) - 1), disable=False):
                ...
                if nnls_exact:
                    ...
                    lasso_noblock = Lasso(
                        ...
                        fit_intercept=False,
                        normalize=False,
                        precompute=xtx_noblock,
                        positive=True,
                        max_iter=10000,
                        random_state=0,
                    )
                    jk_est = lasso_noblock.fit(
                        x_noblock, y_noblock[:, 0]
                    ).coef_.reshape((1, x.shape[1]))
        ...
        if evenodd_split:
            ...
            for chr_i, left_out_chr in enumerate(tqdm(self.chr_list)):
                is_loco = ((chr_num % 2) == (left_out_chr % 2)) & (
                    chr_num != left_out_chr
                )
                x_loco = x[is_loco]
                y_loco = y[is_loco]
...
class LstsqJackknifeFast(Jackknife):
    def __init__(
        ...
        self.delete_values = self.block_values_to_delete_values(
            xty, xtx, xtx_tot=xtx_tot, xty_tot=xty_tot
        )
        self.pseudovalues = self.delete_values_to_pseudovalues(
            self.delete_values, self.est
        )
    ...
    def block_values_to_delete_values(
        cls, xty_block_values, xtx_block_values, xtx_tot, xty_tot
    ):
...
class Jackknife_Ridge(Jackknife):
    def __init__(
        ...
        self.ridge_lambdas = np.logspace(
            np.log10(mean_diag * 1e-8), np.log10(mean_diag * 1e2), num=num_lambdas
        )
        ...
        if ridge_lambda is not None:
            ...
            best_lambda, r2_best_lambda = self._find_best_lambda(
                x, y, XTX_all, XTy_all, chr_num
            )
        ...
        (
            ...
            x, y, XTX_all, XTy_all, chr_num, best_lambda, standardize, x_l2
        )

        # run jackknife
        if not skip_ridge_jackknife:
            self.delete_values = np.empty(
                (len(self.separators) - 1, self.est.shape[1]), dtype=np.float32
            )
            ...
            for block_i in tqdm(range(len(self.separators) - 1)):
                ...
                x_block = x[
                    self.separators[block_i] : self.separators[block_i + 1], ...
                ]
                y_block = y[
                    self.separators[block_i] : self.separators[block_i + 1], ...
                ]
                ...
                if approx_ridge:
                    ...
                    best_lambda_noblock, r2_noblock = self._find_best_lambda(
                        x_noblock, y_noblock, XTX_noblock, XTy_noblock, chr_noblock
                    )
                self.best_r2_jk_noblock[block_i] = r2_noblock

                # main jackknife estimation
                est_block = self._est_ridge(
                    XTX_noblock, XTy_noblock, best_lambda_noblock
                )
    ...
    def _est_taus_loco(
        ...
        chromosomes = np.sort(np.unique(chr_num))
        est_set_lstsq = np.empty(
            (len(self.chromosome_sets), x.shape[1]), dtype=np.float32
        )
        est_noset_lstsq = np.empty(
            (len(self.chromosome_sets), x.shape[1]), dtype=np.float32
        )
        est_set_ridge = np.empty(
            (len(self.chromosome_sets), x.shape[1]), dtype=np.float32
        )
        est_noset_ridge = np.empty(
            (len(self.chromosome_sets), x.shape[1]), dtype=np.float32
        )
        ...
        for set_i, chromosome_set in enumerate(tqdm_chr_sets):
            ...
            if (not reestimate_lambda) or (len(chromosomes) <= 2):
                ...
                best_lambda_noset, r2_noset = self._find_best_lambda(
                    x_loco, y_loco, XTX_noset, XTy_noset, chr_loco
                )
                if len(chromosome_set) == 1:
                    best_lambda_set = ridge_lambda
                else:
                    best_lambda_set, r2_set = self._find_best_lambda(
                        x_set, y_set, XTX_set, XTy_set, chr_num[is_in_set]
                    )
            ...
            est_noset_lstsq[set_i, :] = self._est_ridge(
                XTX_noset, XTy_noset, ridge_lambda=0
            )
            est_noset_ridge[set_i, :] = self._est_ridge(
                XTX_noset, XTy_noset, best_lambda_noset
            )
    ...
    def _find_best_lambda(self, x, y, XTX, XTy, chr_num):
        ...
        for chr_i, left_out_chr in enumerate(tqdm(chromosomes)):
            ...
            y_pred_lambdas[chr_slice, :] = self._predict_lambdas(
                XTX_loco, XTy_loco, x_chr
            )
        ...
        if self.use_1se:
            ...
            for chr_i, left_out_chr in enumerate(chromosomes):
                is_chr = chr_num == left_out_chr
                score_folds[chr_i] = r2_score(
                    y[is_chr], y_pred_lambdas[is_chr, best_lambda_index]
                )
        ...
        if self.verbose:
            ...
            logging.info(
                "Selected ridge lambda: %0.4e (%d/%d)  score: %0.4e  score lstsq: %0.4e"
                % (
                    best_lambda,
                    best_lambda_index + 1,
                    num_lambdas,
    ...
    def _predict_lambdas(self, XTX_train, XTy_train, X_validation):
        tau_est_ridge = np.empty(
            (XTX_train.shape[0], len(self.ridge_lambdas)), dtype=np.float32
        )
