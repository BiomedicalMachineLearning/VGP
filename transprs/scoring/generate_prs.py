...
def generate_prs(processor, method, use_sum=True, validate=True):
    ...
    if validate:
        population = processor.validation
        processor.prs_validation[method] = {}
    else:
        population = processor.test
        processor.prs_test[method] = {}
    ...
    if validate:
        ...
        if use_sum:
            ...
            """
                % (
                    processor.adjusted_ss[method],
                    processor.prs_validation[method]["optimal_pval"],
                    processor.prs_validation[method]["optimal_pval"],
                    population,
                    processor.adjusted_ss[method],
                    str(effect_ind),
    ...
    for filename in snp_results_files:
        ...
        if validate:
            processor.prs_validation[method][new_key] = prs_result
        else:
            processor.prs_test[method][new_key] = prs_result

    if validate:
        print("The PRS result stores in .prs_validation['" + method + "']!")
    else:
        print("The PRS result stores in .prs_test['" + method + "']!")
