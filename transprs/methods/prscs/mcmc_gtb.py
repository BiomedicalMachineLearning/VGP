...
def mcmc(
    ...
    for itr in range(1, n_iter + 1):
            ...
            for kk in range(n_blk[pp]):
                if blk_size[pp][kk] == 0:
                    ...
                    beta_tmp = linalg.solve_triangular(
                        dinvt_chol, beta_mrg[pp][idx_blk], trans="T"
                    ) + sp.sqrt(sigma[pp] / n[pp]) * random.randn(len(idx_blk), 1)
                    beta[pp][idx_blk] = linalg.solve_triangular(
                        dinvt_chol, beta_tmp, trans="N"
                    )
                    quad += sp.dot(
                        sp.dot(beta[pp][idx_blk].T, dinvt), beta[pp][idx_blk]
                    )
                    mm += blk_size[pp][kk]

            err = max(
                n[pp] / 2.0 * (1.0 - 2.0 * sum(beta[pp] * beta_mrg[pp]) + quad),
                n[pp] / 2.0 * sum(beta[pp] ** 2 / psi_pp),
            )
        ...
        for jj in range(p_tot):
            while True:
                try:
                    psi[jj] = gigrnd.gigrnd(
                        a - 0.5 * n_grp[jj], 2.0 * delta[jj], xx[jj]
                    )
                except:
                    continue
                else:
    ...
    if meta == "TRUE":
        ...
        for pp in range(n_pop):
            vv[idx_dict[pp]] += (
                n[pp] / sigma_est[pp] / psi_est[idx_dict[pp]] * het[pp] ** 2
            )
            zz[idx_dict[pp]] += (
                n[pp]
                / sigma_est[pp]
    ...
    for pp in range(n_pop):
        if phi_updt == True:
            eff_file = (
                out_dir
                + "/"
                + "%s_%s_pst_eff_a%d_b%.1f_phiauto_chr%d.txt"
                % (out_name, pop[pp], a, b, chrom)
            )
        else:
            eff_file = (
                out_dir
                + "/"
                + "%s_%s_pst_eff_a%d_b%.1f_phi%1.0e_chr%d.txt"
                % (out_name, pop[pp], a, b, phi, chrom)
            )
    ...
    if meta == "TRUE":
        if phi_updt == True:
            eff_file = (
                out_dir
                + "/"
                + "%s_META_pst_eff_a%d_b%.1f_phiauto_chr%d.txt"
                % (out_name, a, b, chrom)
            )
        else:
            eff_file = (
                out_dir
                + "/"
                + "%s_META_pst_eff_a%d_b%.1f_phi%1.0e_chr%d.txt"
                % (out_name, a, b, phi, chrom)
            )

        with open(eff_file, "w") as ff:
            for snp, bp, a1, a2, beta in zip(
                snp_dict["SNP"], snp_dict["BP"], snp_dict["A1"], snp_dict["A2"], mu
            ):
                ff.write("%d\t%s\t%d\t%s\t%s\t%.6e\n" % (chrom, snp, bp, a1, a2, beta))
