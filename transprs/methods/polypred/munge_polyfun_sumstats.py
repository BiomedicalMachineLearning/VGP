...
def find_df_column(df, strings_to_find, allow_missing=False):
    ...
    for str_to_find in strings_to_find:
        is_relevant_col = is_relevant_col | (
            df.columns.str.upper() == str_to_find.upper()
        )
    if np.sum(is_relevant_col) == 0:
        if allow_missing:
            ...
            raise ValueError(
                "No matching column found among: %s" % str(strings_to_find)
            )
    elif np.sum(is_relevant_col) > 1:
        raise ValueError(
            "Too many matching columns found among: %s" % str(strings_to_find)
        )
    else:
...
def rename_df_columns(df_sumstats):
    ...
    bp_column = find_df_column(
        df_sumstats, ["BP", "POS", "POSITION", "COORDINATE", "BASEPAIR"]
    )
    snp_column = find_df_column(
        df_sumstats, ["SNP", "RSID", "RS", "NAME", "MarkerName"]
    )
    a1freq_col = find_df_column(
        df_sumstats, ["A1FREQ", "freq", "MAF", "FRQ"], allow_missing=True
    )
    info_col = find_df_column(df_sumstats, "INFO", allow_missing=True)
    beta_col = find_df_column(
        df_sumstats, ["BETA", "EFF", "EFFECT", "EFFECT_SIZE", "OR"], allow_missing=True
    )
    se_col = find_df_column(df_sumstats, ["SE"], allow_missing=True)
    pvalue_col = find_df_column(
        df_sumstats,
        ["P_BOLT_LMM", "P", "PVALUE", "P-VALUE", "P_value", "PVAL"],
        allow_missing=True,
    )
    ...
    ncase_col = find_df_column(
        df_sumstats, ["N_cases", "Ncase", "Nca", "Total_NCase"], allow_missing=True
    )
    ncontrol_col = find_df_column(
        df_sumstats,
        ["N_controls", "Ncontrol", "Nco", "Total_NControl"],
        allow_missing=True,
    )
    try:
...
def compute_z(df_sumstats):
    ...
    if "BETA" not in df_sumstats.columns:
        raise ValueError(
            "Beta column not found in sumstats file (required to compute Z-scores)"
        )
    if "P" not in df_sumstats.columns:
        raise ValueError(
            "P-value column not found in sumstats file (required to compute Z-scores)"
        )

    # compute z-scores
    df_sumstats["Z"] = stats.norm(0, 1).isf(df_sumstats["P"] / 2.0) * np.sign(
        df_sumstats["BETA"]
    )
    ...
    if np.any(is_zero_pval):
        ...
        if "MAF" not in df_sumstats.columns:
            raise ValueError(
                "MAF column not found in sumstats file (required to compute Z-scores for SNPs with P=0)"
            )

        # estimate sigma2pheno
        df_sumstats_nonzero = df_sumstats.loc[~is_zero_pval]
        df_snp_var_nonzero = (
            2 * df_sumstats_nonzero["MAF"] * (1 - df_sumstats_nonzero["MAF"])
        )
        ...
        df_snp_var_zero = (
            2 * df_sumstats_iszero["MAF"] * (1 - df_sumstats_iszero["MAF"])
        )
        df_sumstats.loc[is_zero_pval, "Z"] = (
            df_sumstats_iszero["BETA"] * np.sqrt(df_snp_var_zero) * sqrt_sigma2pheno
        )
...
def filter_sumstats(
    ...
    if "CHISQ_BOLT_LMM" in df_sumstats.columns:
        ...
        if np.any(~is_good_chi2_snp):
            logging.info(
                "Removing %d SNPs with BOLT CHI2=0" % (np.sum(~is_good_chi2_snp))
            )
    ...
    if min_info_score is not None and min_info_score > 0:
        if "INFO" not in df_sumstats.columns:
            logging.warning(
                "Could not find INFO column. Please set --min-info 0 to omit this warning."
            )
        else:
            ...
            if np.any(~is_good_info_snp):
                logging.info(
                    "Removing %d SNPs with INFO<%0.2f"
                    % (np.sum(~is_good_info_snp), min_info_score)
                )
    ...
    if min_maf is not None and min_maf > 0:
        if "MAF" not in df_sumstats.columns:
            logging.warning(
                "Could not find MAF column. Please set --min-maf 0 to omit this warning."
            )
        else:
            ...
            if np.any(~is_good_maf_snp):
                logging.info(
                    "Removing %d SNPs with MAF<%s" % (np.sum(~is_good_maf_snp), min_maf)
                )
    ...
    if remove_strand_ambig:
        ...
        for ambig_pairs in [("A", "T"), ("T", "A"), ("C", "G"), ("G", "C")]:
            is_strand_ambig = is_strand_ambig | (
                (df_sumstats["A2"] == ambig_pairs[0])
                & (df_sumstats["A1"] == ambig_pairs[1])
            )
        is_good_snp = is_good_snp & (~is_strand_ambig)
        if np.any(is_strand_ambig):
            logging.info(
                "Removing %d SNPs with strand ambiguity" % (is_strand_ambig.sum())
            )

    # remove HLA SNPs
    if not keep_hla:
        is_hla = (df_sumstats["CHR"] == 6) & (
            df_sumstats["BP"].between(28000000, 34000000)
        )
    ...
    if np.any(~is_good_snp):
        ...
        logging.info(
            "%d SNPs with sumstats remained after all filtering stages"
            % (df_sumstats.shape[0])
        )
...
def convert_odds_ratio_to_log(df_sumstats):
    ...
    if np.any(df_sumstats["OR"] == 0):
        raise ValueError(
            "The input file includes SNPs with an odds ratio (OR) of 0. Please remove these variant(s)."
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--sumstats", required=True, help="Input summary statistics file"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Sample size. If not specified, will try to infer this from the input file",
    )
    parser.add_argument(
        "--min-info",
        type=float,
        default=0.6,
        help="Minimum INFO score (set to zero to avoid INFO-based filtering)",
    )
    parser.add_argument(
        "--min-maf",
        type=float,
        default=0.001,
        help="Minimum MAF (set to zero to avoid MAF-based filtering)",
    )
    parser.add_argument(
        "--remove-strand-ambig",
        default=False,
        action="store_true",
        help="If specified, strand-ambigous SNPs will be removed",
    )
    parser.add_argument(
        "--chi2-cutoff",
        type=float,
        default=30,
        help="Chi2 cutoff for effective sample size computations",
    )
    parser.add_argument(
        "--keep-hla",
        default=False,
        action="store_true",
        help="If specified, Keep SNPs in the HLA region",
    )
    parser.add_argument(
        "--no-neff",
        default=False,
        action="store_true",
        help="If specified, use the true rather than the effective sample size in BOLT-LMM runs",
    )
    ...
    # filter sumstats
    df_sumstats = filter_sumstats(
        df_sumstats,
        min_info_score=args.min_info,
        min_maf=args.min_maf,
        remove_strand_ambig=args.remove_strand_ambig,
    ...
    if "CHISQ_BOLT_LMM" in df_sumstats.columns and not args.no_neff:
        ...
        if "N" in df_sumstats.columns:
            raise ValueError(
                "cannot both specify --n and have an N column in the sumstats file"
            )
        if "N_CASES" in df_sumstats.columns or "N_CONTROLS" in df_sumstats.columns:
            raise ValueError(
                "cannot both specify --n and have an N_cases/N_controls column in the sumstats file"
            )
        df_sumstats["N"] = args.n
    elif "N" in df_sumstats.columns:
        if "N_CASES" in df_sumstats.columns or "N_CONTROLS" in df_sumstats.columns:
            raise ValueError(
                "cannot both have an N column and N_cases/N_controls columns in the sumstats file"
            )
        pass
        ...
        raise ValueError(
            "must specify sample size, via either (1) --n flag; (2) N column in the sumstats file; or (3) Two columns N_cases, N_controls in the sumstats file"
        )
    ...
    if "Z" in df_sumstats.columns:
        ...
        df_sumstats["Z"] = np.sqrt(df_sumstats["CHISQ_BOLT_LMM"]) * np.sign(
            df_sumstats["BETA"]
        )
        ...
        raise ValueError(
            "Sumstats file must include a p-value, Z-score or chi2 column to compute Z-scores"
        )

    # write output
    logging.info(
        "Saving munged sumstats of %d SNPs to %s" % (df_sumstats.shape[0], args.out)
    )
    if "MAF" in df_sumstats.columns:
        df_sumstats.loc[df_sumstats["MAF"] > 0.5, "MAF"] = (
            1 - df_sumstats.loc[df_sumstats["MAF"] > 0.5, "MAF"]
        )
    columns_to_keep = [
        c
        for c in df_sumstats.columns
        if c in ["SNP", "CHR", "BP", "A1", "A2", "Z", "N", "MAF"]
    ]
