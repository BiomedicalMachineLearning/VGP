...
def compute_ldscores(args):
    ...
    if len(geno_array.kept_snps) < df_bim.shape[0]:
        assert np.all(
            np.array(geno_array.kept_snps) == np.sort(np.array(geno_array.kept_snps))
        )
    ...
    annot_values = (
        None if args.annot is None else df_annot.drop(columns=SNP_COLUMNS).values
    )
    ldscores = geno_array.ldScoreVarBlocks(
        block_left, args.chunk_size, annot=annot_values
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--annot",
        default=None,
        help="annotations file (if not specified, compute the global LD-score)",
    )
    parser.add_argument(
        "--ld-wind-cm",
        type=float,
    ...
    parser.add_argument(
        "--keep", default=None, help="File with ids of individuals to use"
    )
    parser.add_argument(
        "--allow-missing",
        default=False,
        action="store_true",
        help="If specified, The script will not terminate if some SNPs with sumstats are not found in the annotations files",
    )
