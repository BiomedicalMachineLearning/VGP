...
def parse_ref(ref_file, chrom, ref):
    ...
    print(
        "... %d SNPs on chromosome %d read from %s ..."
        % (len(ref_dict["SNP"]), chrom, ref_file)
    )
    return ref_dict
...
def parse_bim(bim_file, chrom):
    ...
    print(
        "... %d SNPs on chromosome %d read from %s ..."
        % (len(vld_dict["SNP"]), chrom, bim_file + ".bim")
    )
    return vld_dict
...
def parse_sumstats(ref_dict, vld_dict, sst_file, pop, n_subj):
    ...
    ref_snp = (
        set(zip(snp_ref, a1_ref, a2_ref))
        | set(zip(snp_ref, a2_ref, a1_ref))
        | set(
            zip(snp_ref, [mapping[aa] for aa in a1_ref], [mapping[aa] for aa in a2_ref])
        )
        | set(
            zip(snp_ref, [mapping[aa] for aa in a2_ref], [mapping[aa] for aa in a1_ref])
        )
    )
    ...
    sst_snp = (
        ...
        | set(
            zip(
                sst_dict["SNP"],
                [mapping[aa] for aa in sst_dict["A1"]],
                [mapping[aa] for aa in sst_dict["A2"]],
            )
        )
        | set(
            zip(
    ...
    print(
        "... %d common SNPs in the %s reference, %s sumstats, and validation set ..."
        % (len(comm_snp), pop.upper(), pop.upper())
    )
...
def parse_ldblk(ldblk_dir, sst_dict, pop, chrom, ref):
    ...
    n_blk = len(hdf_chr)
    ld_blk = [
        sp.array(hdf_chr["blk_" + str(blk)]["ldblk"]) for blk in range(1, n_blk + 1)
    ]

    snp_blk = []
    for blk in range(1, n_blk + 1):
        snp_blk.append(
            [bb.decode("UTF-8") for bb in list(hdf_chr["blk_" + str(blk)]["snplist"])]
        )
...
def align_ldblk(ref_dict, vld_dict, sst_dict, n_pop, chrom):
    ...
    for pp in range(n_pop):
        ...
        idx_dict[pp] = [
            ii for (ii, snp) in enumerate(snp_dict["SNP"]) if snp in sst_dict[pp]["SNP"]
        ]
