...
class Fine_Mapping(object):
    ...
    def sync_ld_sumstats(self, ld_arr, df_ld_snps, allow_missing=False):
        ...
        if ld_arr is None:
            df_ld = pd.DataFrame(
                np.zeros(len(df_ld_snps.index), dtype=np.int),
    ...
    def compute_ld_bgen(self, locus_start, locus_end, verbose=False):
        ...
        for snp_i, rsid in enumerate(rsids):
            if rsid not in df_z["SNP"].values:
                continue
        ...
        bcor_file = (
            self.get_ld_output_file_prefix(locus_start, locus_end, temp_dir) + ".bcor"
        )
...
class SUSIE_Wrapper(Fine_Mapping):
    ...
    def finemap(
        ...
        # mark causal sets
        self.susie_dict = {
            key: np.array(susie_obj.rx2(key), dtype=np.object)
...
class FINEMAP_Wrapper(Fine_Mapping):
    ...
    def finemap(
        ...
        if num_causal_snps == 1:
            ...
            np.savetxt(
                ld_file, np.eye(self.df_sumstats_locus.shape[0], dtype=np.int), fmt="%s"
            )
        else:
            if ld_file is None:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--susie-outfile",
        default=None,
        help="If specified, the SuSiE object will be saved to an output file",
    )
    parser.add_argument(
        "--finemap-dir",
        default=None,
