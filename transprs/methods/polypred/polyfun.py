...
def check_args(args):
    ...
    mode_params = np.array(
        [args.compute_h2_L2, args.compute_ldscores, args.compute_h2_bins]
    )
    if np.sum(mode_params) == 0:
        raise ValueError(
            "must specify at least one of --compute-h2-L2, --compute-ldscores, --compute-h2-bins"
        )
    if args.compute_h2_L2 and args.compute_h2_bins and not args.compute_ldscores:
        raise ValueError(
            "cannot use both --compute-h2_L2 and --compute_h2_bins without also specifying --compute-ldscores"
        )
    if args.chr is not None:
        if args.compute_h2_L2 or args.compute_h2_bins:
            raise ValueError(
                "--chr can only be specified when using only --compute-ldscores"
            )
    if args.bfile_chr is not None:
        if not args.compute_ldscores:
            raise ValueError(
                "--bfile-chr can only be specified when using --compute-ldscores"
            )
    if args.ld_ukb:
        if not args.compute_ldscores:
            raise ValueError(
                "--ld-ukb can only be specified when using --compute-ldscores"
            )
    if args.no_partitions:
        if not args.compute_h2_L2:
            raise ValueError(
                "cannot specify --no-partitions without specifying --compute-h2-L2"
            )
        if args.compute_ldscores:
            raise ValueError(
                "cannot specify both --no-partitions and --compute-ldscores"
            )
        if args.compute_h2_bins:
            raise ValueError(
                "cannot specify both --no-partitions and --compute-h2-bins"
            )
    if args.compute_ldscores and args.compute_h2_bins and not args.compute_h2_L2:
        raise ValueError(
            "cannot use both --compute-ldscores and --compute_h2_bins without also specifying --compute-h2-L2"
        )
    ...
    if args.compute_ldscores:
        if args.bfile_chr is None and not args.ld_ukb:
            raise ValueError(
                "You must specify either --bfile-chr or --ld-ukb when you specify --compute-ldscores"
            )
        if not args.ld_ukb and (
            args.ld_wind_cm is None
            and args.ld_wind_kb is None
            and args.ld_wind_snps is None
        ):
            args.ld_wind_cm = 1.0
            logging.warning(
                "no ld-wind argument specified.  PolyFun will use --ld-cm 1.0"
            )
    ...
    if not args.compute_ldscores:
        if not (
            ...
            raise ValueError(
                "--ld-wind parameters can only be specified together with --compute-ldscores"
            )
        if args.keep is not None:
            raise ValueError(
                "--keep can only be specified together with --compute-ldscores"
            )
        if args.chr is not None:
            raise ValueError(
                "--chr can only be specified together with --compute-ldscores"
            )
    ...
    if args.compute_h2_L2:
        ...
        if args.ref_ld_chr is None:
            raise ValueError(
                "--ref-ld-chr must be specified when using --compute-h2-L2"
            )
    ...
    if args.compute_h2_bins:
        if args.sumstats is None:
            raise ValueError(
                "--sumstats must be specified when using --compute-h2-bins"
            )
        if args.w_ld_chr is None:
            raise ValueError(
                "--w-ld-chr must be specified when using --compute-h2-bins"
            )
        if args.ref_ld_chr is not None and not args.compute_ldscores:
            raise ValueError(
                "--ref-ld-chr should not be specified when using --compute-h2-bins, unless you also use --compute-ldscores"
            )
...
def check_files(args):
    ...
    if args.compute_h2_L2:
        ...
        for chr_num in range(1, 23):
            get_file_name(
                args, "ref-ld", chr_num, verify_exists=True, allow_multiple=True
            )
            get_file_name(args, "w-ld", chr_num, verify_exists=True)
            get_file_name(
                args, "annot", chr_num, verify_exists=True, allow_multiple=True
            )
...
class PolyFun:
    ...
    def run_ldsc(self, args, use_ridge, nn, keep_large, evenodd_split, n_blocks=2):
        ...
        if use_ridge or not args.compute_ldscores or True:
            (
                ...
                df_sumstats,
                _,
            ) = sumstats._read_ld_sumstats(args, log, args.h2)
        else:
        ...
        if df_sumstats.shape[0] < 200000:
            logging.warning(
                "number of SNPs is smaller than 200k; this is almost always bad."
            )
    ...
    def load_annotations_file(self, args, chr_num, use_ridge):
        ...
        if len(df_annot_chr_list) == 1:
            ...
            for df in df_annot_chr_list[1:]:
                for snp_col in SNP_COLUMNS:
                    if (df.shape[0] != df_annot_chr_list[0].shape[0]) or (
                        np.any(df[snp_col] != df_annot_chr_list[0][snp_col])
                    ):
                        raise ValueError(
                            "Different annotation files of chromosome %d must be perfectly aligned"
                            % (chr_num)
                        )
        ...
        if not np.all(np.isin(self.ref_ld_cnames, df_annot_chr.columns)):
            raise ValueError(
                "Annotation names in annotations file do not match the one in the LD-scores file"
            )
        ...
        if not np.all(
            [c for c in df_annot_chr.columns if c not in SNP_COLUMNS]
            == self.ref_ld_cnames
        ):
            raise ValueError(
                "Annotation names in annotations file do not match the one in the LD-scores file"
    ...
    def compute_snpvar_chr(self, args, chr_num, use_ridge):
        ...
        if use_ridge:
            ...
            found_chrom = np.any(
                [chr_num in chr_set for chr_set in jknife.chromosome_sets]
            )
            if not found_chrom:
                raise ValueError(
                    "not all chromosomes have a taus estimate - please make sure that the intersection of SNPs with sumstats and with annotations data spans all 22 human chromosomes"
                )
            ...
            if set_num is None:
                raise ValueError(
                    "Could not find Ridge predictions for chromosome %d" % (chr_num)
                )
        ...
        taus_output_file = get_file_name(
            args,
            ("taus_ridge" if use_ridge else "taus_nn"),
            chr_num,
            verify_exists=False,
        )
        df_taus = pd.Series(
            taus, index=df_annot_chr.drop(columns=SNP_COLUMNS, errors="raise").columns
        )
    ...
    def create_df_bins(
        self, bin_sizes, df_snpvar, df_snpvar_sorted=None, min_bin_size=10
    ):
        ...
        while True:
            ...
            num_snps_to_transfer = np.minimum(
                min_bin_size - bin_sizes[bin_i], bin_sizes[bin_i - 1]
            )
    ...
    def partition_snps_Ckmedian(self, args, use_ridge):
        ...
        try:
            ...
            logging.error(
                "Could not load the R package Ckmeans.1d.dp. Either install it or rerun PolyFun with --skip-Ckmedian"
            )
            logging.error("")
            raise
        ...
        if args.num_bins is None or args.num_bins <= 0:
            logging.info(
                "Determining the optimal number of bins (if this is slow, consider using --num-bins 20 (or some other number))"
            )
    ...
    def partition_snps_Kmeans(self, args, use_ridge):
        logging.info(
            "Clustering SNPs into bins using K-means clustering with %d bins"
            % (args.num_bins)
        )
        ...
        for bin_i, cluster_label in enumerate(bins_order[:-1]):
            ...
            assert (
                df_snpvar.loc[kmeans_obj.labels_ == cluster_label, "SNPVAR"].max()
                <= df_snpvar.loc[
                    kmeans_obj.labels_ == next_cluster_label, "SNPVAR"
                ].min()
            )
    ...
    def save_snpvar_to_disk(self, args, use_ridge, constrain_range):
        ...
        for chr_num in tqdm(range(1, 23)):
            ...
            if use_ridge:
                output_fname += "_ridge"
            if constrain_range:
                output_fname += "_constrained"
            snpvar_chr_file = get_file_name(
                args, output_fname, chr_num, verify_exists=False
            )

            # save snpvar to file
            df_snpvar_chr = df_snpvar.query("CHR==%d" % (chr_num))
            df_snpvar_chr.to_csv(
                snpvar_chr_file,
                index=False,
                sep="\t",
                compression="gzip",
    ...
    def polyfun_h2_L2(self, args):
        ...
        self.run_ldsc(
            args, use_ridge=True, nn=False, evenodd_split=False, keep_large=False
        )
    ...
    def compute_ld_scores(self, args):
        ...
        for chr_num in tqdm(chr_range, disable=len(chr_range) == 1):
            ...
            if args.ld_ukb:
                ...
                df_ldscores_chr = compute_ldscores_chr(
                    df_bins_chr, ld_dir=ld_dir, use_ukb=True
                )
            ...
            ldscores_output_file = get_file_name(
                args, "ldscores", chr_num, verify_exists=False
            )
    ...
    def compute_ldscores_plink_chr(self, args, chr_num, df_bins_chr):
        ...
        if np.any(~df_bim.index.isin(df_bins_chr.index)):
            ...
            if args.allow_missing:
                ...
                if not np.any(is_good_snp):
                    raise ValueError(
                        "No SNPs in chromosome %d have annotations" % (chr_num)
                    )
        ...
        if args.keep is None:
            ...
            logging.info(
                "after applying --keep, %d individuals remain" % (len(keep_indivs))
            )

        # read plink file
        logging.info("Loading SNP file...")
        bed_file = get_file_name(args, "bed", chr_num)
        geno_array = ldscore.PlinkBEDFile(
            bed_file,
            n,
            array_snps,
            keep_snps=keep_snps,
        ...
        if len(geno_array.kept_snps) != df_bim.shape[0]:
            assert np.all(
                ...
                == np.sort(np.array(geno_array.kept_snps))
            )
    ...
    def compute_h2_bins(self, args, constrain_range):
        # run S-LDSC
        self.run_ldsc(
            args, use_ridge=False, nn=True, evenodd_split=True, keep_large=False
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Number of bins to partition SNPs into. If not specified, PolyFun will automatically select this number based on a BIC criterion",
    )
    parser.add_argument(
        "--anno",
        default=None,
        help="Comma-delimited list of annotations to use (if not specified, will use all annotations)",
    )
    parser.add_argument(
        "--skip-Ckmedian",
        default=False,
        action="store_true",
        help="If specified, use a regular K-means algorithm instead of the R Ckmeans.1d.dp package",
    )
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="If specified, PolyFun will compute LD-scores of SNP bins",
    )
    parser.add_argument(
        "--compute-h2-L2",
        default=False,
        action="store_true",
        help="If specified, PolyFun will compute per-SNP h2 using L2-regularized S-LDSC",
    )
    parser.add_argument(
        "--compute-h2-bins",
        default=False,
        action="store_true",
        help="If specified, PolyFun will robustly compute per-SNP h2 based on SNP bins",
    )
    parser.add_argument(
        "--no-partitions",
        default=False,
        action="store_true",
        help="If specified, PolyFun will not partition SNPs into bins (do this only if you want to use the per-SNP h^2 from L2-regularized S-LDSC as prior causal probabilities, which is typically not recommended)",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Chromosome number (only applicable when only specifying --ldscores). If not set, PolyFun will compute LD-scores for all chromosomes",
    )
    parser.add_argument(
        "--ld-wind-cm",
        type=float,
        default=None,
        help="window size to be used for estimating LD-scores in units of centiMorgans (cM).",
    )
    parser.add_argument(
        "--ld-wind-kb",
        type=int,
        default=None,
        help="window size to be used for estimating LD-scores in units of Kb.",
    )
    parser.add_argument(
        "--ld-wind-snps",
        type=int,
        default=None,
        help="window size to be used for estimating LD-scores in units of SNPs.",
    )
    parser.add_argument(
        "--chunk-size",
        type=int,
        default=50,
        help="chunk size for LD-scores calculation",
    )
    parser.add_argument(
        "--keep",
        default=None,
        help="File with ids of individuals to use when computing LD-scores",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=100,
        help="The maximum ratio between the largest and smallest truncated per-SNP heritabilites",
    )
    ...
    parser.add_argument(
        "--w-ld-chr", help="Suffix of LD-score weights files (as in ldsc)"
    )
    parser.add_argument(
        "--bfile-chr",
        default=None,
        help="Prefix of plink files (used to compute LD-scores)",
    )
    parser.add_argument(
        "--ld-ukb",
        default=False,
        action="store_true",
        help="If specified, PolyFun will use UKB LD matrices to compute LD-scores",
    )
    parser.add_argument(
        "--ld-dir",
        default=None,
        help="The path of a directory with UKB LD files (if not specified PolyFun will create a temporary directory)",
    )
    parser.add_argument(
        "--output-prefix", required=True, help="Prefix of all PolyFun output file names"
    )
    parser.add_argument(
        "--allow-missing",
        default=False,
        action="store_true",
        help="If specified, PolyFun will not terminate if some SNPs with sumstats are not found in the annotations files",
    )
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="If specified, S-LDSC will estimate non-negative taus using an exact instead of an approximate solver (this will be slower but slightly more accurate)",
    )
    ...
    if len(os.path.dirname(args.output_prefix)) > 0 and not os.path.exists(
        ...
        raise ValueError(
            "output directory %s doesn't exist" % (os.path.dirname(args.output_prefix))
        )
