...
def parse_param():
    ...
    if param_dict["ref_dir"] == None:
        ...
        print(
            "* Please specify the directory and prefix of the bim file for the target (validation/testing) dataset using --bim_prefix\n"
        )
        sys.exit(2)
    elif param_dict["sst_file"] == None:
        print(
            "* Please provide at least one summary statistics file using --sst_file\n"
        )
        sys.exit(2)
        ...
        sys.exit(2)
    elif len(param_dict["sst_file"]) != len(param_dict["n_gwas"]) or len(
        param_dict["sst_file"]
    ) != len(param_dict["pop"]):
    ...
    for chrom in param_dict["chrom"]:
        ...
        if os.path.isfile(param_dict["ref_dir"] + "/snpinfo_mult_1kg_hm3"):
            ...
            ref_dict = parse_genet.parse_ref(
                param_dict["ref_dir"] + "/snpinfo_mult_1kg_hm3", int(chrom), ref
            )
        elif os.path.isfile(param_dict["ref_dir"] + "/snpinfo_mult_ukbb_hm3"):
            ref = "ukbb"
            ref_dict = parse_genet.parse_ref(
                param_dict["ref_dir"] + "/snpinfo_mult_ukbb_hm3", int(chrom), ref
            )
        ...
        for pp in range(n_pop):
            sst_dict[pp] = parse_genet.parse_sumstats(
                ref_dict,
                vld_dict,
                param_dict["sst_file"][pp],
                param_dict["pop"][pp],
                param_dict["n_gwas"][pp],
            )
