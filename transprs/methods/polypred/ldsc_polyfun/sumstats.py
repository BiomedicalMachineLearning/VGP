...
def _check_variance(log, M_annot, ref_ld):
    ...
    ii = np.array(
        [(ref_ld[c].var() == 0) for c in ref_ld.columns[2:]]
    )  # This command uses way way less memory
    if ii.all():
        raise ValueError("All LD Scores have zero variance.")
    elif ii.any():
        log.log(
            "Removing partitioned LD Scores with zero variance: %s"
            % (",".join(ref_ld.columns[2:][ii]))
        )
...
def _read_ld_sumstats(args, log, fh, alleles=True, dropna=True):
    ...
    n_annot = (
        len(ref_ld.columns) - 2
    )  # Changed to -2 because we also have chromosome column now
    ...
    if args.anno is not None:
        ...
        if np.any(~is_found):
            raise ValueError(
                "Not all annotations specified with --anno are found in the LD scores file"
            )
        cols_to_keep = (
            (ref_ld.columns.str[:-2].isin(annotations))
            | (ref_ld.columns.str[:-4].isin(annotations))
            | (ref_ld.columns.isin(["CHR", "SNP"]))
        )
        assert np.sum(cols_to_keep) == len(annotations) + 2
        cols_nochrsnp = ref_ld.drop(columns=["CHR", "SNP"]).columns
        M_cols_to_keep = (cols_nochrsnp.str[:-2].isin(annotations)) | (
            cols_nochrsnp.str[:-4].isin(annotations)
        )
...
def estimate_h2(args, log):
    ...
    if args.loco and args.ridge_jackknife and args.reestimate_lambdas:
        np.savetxt(
            args.out + ".out_of_chrom_r2.txt", [hsqhat.jknife_ridge.r2_best_lambda]
        )
...
def _get_rg_table(rg_paths, RG, args):
    ...
    if (
        ...
        x["h2_liab_se"] = [
            c * x for x in list(map(t("tot_se"), list(map(t("hsq2"), RG))))
        ]
    else:
