...
def read_input(
    ...
    validation_phenotype=None,
):
    # Read the population file

    print("Reading the Test genotype...")

    test = read_plink1_bin(
        prefix_test + ".bed", prefix_test + ".bim", prefix_test + ".fam"
    )
    test.name = "test"

    if prefix_validation is not None:
        print("Reading the Validation genotype...")
        validation = read_plink1_bin(
            prefix_validation + ".bed",
    ...
    processor = tprs.datasets.DataProcessor(
        sumstats=sumstats, test=test, validation=validation, workdir=workdir
    )
