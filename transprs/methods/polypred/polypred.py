...
def create_plink_range_file(df_betas, temp_dir, num_jk=200):
    ...
    ranges_list = [
        {"block": "block%d" % (block), "lb": str(block), "ub": str(block + 1)}
        for block in range(1, num_jk + 1)
    ]
...
def compute_prs_for_file(
    args, plink_file, df_betas, temp_dir, ranges_file=None, keep_file=None
):

    # read the bim file
    plink_file_prefix = plink_file[: plink_file.rfind(".")]
    df_bim = pd.read_csv(
        plink_file_prefix + ".bim",
        header=None,
    ...
    df_betas = df_betas.merge(
        df_bim[["SNP"]], left_index=True, right_index=True, suffixes=("_betas", "_bim")
    )
    if df_betas.shape[0] == 0:
        raise ValueError(
            "No betas found for SNPs in plink file %s" % (plink_file_prefix)
        )
    ...
    df_betas[["SNP_bim", "A1", "BETA"]].to_csv(
        betas_file, header=False, index=False, sep="\t", float_format="%0.8e"
    )
    ...
    if ranges_file is not None:
        ...
        df_betas[["SNP_bim", "score"]].drop_duplicates("SNP_bim").to_csv(
            scores_file, sep="\t", header=False, index=False
        )
    ...
    if ranges_file is None:
        if plink_file.endswith(".bed"):
            ...
            df_prs.rename(
                columns={"#IID": "IID", "SCORE1_SUM": "SCORESUM"}, inplace=True
            )
...
def computs_prs_all_files(args, betas_file, disable_jackknife=False, keep_file=None):
    ...
    for file in args.files:
        if file.endswith(".bed"):
            if found_pgen:
                raise ValueError("Cannot combined .bed and .pgen files")
            if args.plink_exe is None:
                raise ValueError("You must provide --plink-exe if you use .bed files")
            found_bed = True
        elif file.endswith(".pgen"):
            if found_bed:
                raise ValueError("Cannot combined .bed and .pgen files")
            if args.plink2_exe is None:
                raise ValueError("You must provide --plink2-exe if you use .pgen files")
            found_pgen = True
        else:
    ...
    for plink_file in tqdm_files:
        ...
        df_prs_file = compute_prs_for_file(
            args,
            plink_file,
            df_betas,
            temp_dir,
        ...
        if df_prs_sum is None:
            ...
            for c in df_prs_file.columns:
                if not c.startswith("SCORESUM"):
                    continue
                if not c in df_prs_sum.columns:
                    df_prs_sum[c] = 0
                df_prs_sum[c] += df_prs_file[c]

    # compute jackknife-block PRS
    if args.num_jk > 0 and not disable_jackknife:
        jk_columns = df_prs_sum.columns[
            df_prs_sum.columns.str.startswith("SCORESUM.jk")
        ]
        assert np.allclose(
            df_prs_sum[jk_columns].sum(axis=1), df_prs_sum["SCORESUM"], atol=1e-3
        )
        for c in jk_columns:
...
def estimate_mixing_weights(args):
    ...
    for betas_file in beta_files:
        df_prs_sum = computs_prs_all_files(
            args, betas_file, disable_jackknife=True, keep_file=args.pheno
        )
    ...
    if df_pheno.shape[0] != df_prs_sum_all.shape[0] or np.any(
        df_prs_sum_all.index != df_pheno.index
    ):
        df_pheno = df_pheno.loc[df_prs_sum_all.index]

    # compute mixing weights
    mix_weights, intercept = nonneg_lstsq(
        df_prs_sum_all.values, df_pheno["PHENO"].values
    )
    ...
    for betas_file, mix_weight in zip(beta_files, mix_weights):
        ...
        df_new = df_betas_weighted.loc[index_shared].merge(
            df_betas.loc[index_shared, ["BETA2"]], left_index=True, right_index=True
        )
        ...
        df_list = [
            df_new,
            df_betas.loc[~df_betas.index.isin(index_shared)],
            df_betas_weighted.loc[~df_betas_weighted.index.isin(index_shared)],
        ]
        df_betas_weighted = pd.concat(df_list, axis=0)
    df_betas_weighted.sort_values(["CHR", "BP", "A1"], inplace=True)

    # save output to file
    df_betas_weighted[["SNP", "CHR", "BP", "A1", "A2", "BETA"]].to_csv(
        args.output_prefix + ".betas", sep="\t", index=False, float_format="%0.6e"
    )
...
def compute_prs(args):
    ...
    df_prs_sum = computs_prs_all_files(
        args, args.betas, disable_jackknife=False, keep_file=args.keep
    )
    ...
    df_prs_sum_jk = df_prs_sum[
        ["FID", "IID"] + [c for c in df_prs_sum.columns if c.startswith("PRS.")]
    ]

    df_prs_sum_main.to_csv(
        args.output_prefix + ".prs", sep="\t", index=False, float_format="%0.5f"
    )
    if df_prs_sum_jk.shape[1] > 1:
        df_prs_sum_jk.to_csv(
            args.output_prefix + ".prs_jk", sep="\t", index=False, float_format="%0.5f"
        )
...
def check_args(args):
    if int(args.predict) + int(args.combine_betas) != 1:
        raise ValueError(
            "you must specify either --predict or --combine-betas (but not both)"
        )
    ...
    if args.combine_betas:
        ...
        if args.betas.count(",") == 0:
            raise ValueError(
                "you must provide multiple files in --betas if you specify --combine-betas"
            )
    if args.num_jk < 0:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--betas",
        required=True,
        help="files with SNP effect sizes (comma separated). A1 is the effect allele.",
    )
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="If specified, PolyPred will estimate mixing weights",
    )
    parser.add_argument(
        "--predict",
        default=False,
        action="store_true",
        help="If specified, PolyPred will compute PRS",
    )
    parser.add_argument(
        "--pheno",
        default=None,
        help="Phenotype file (required for estimating mixing weights)",
    )
    parser.add_argument(
        "--plink-exe",
        default=None,
        help="Plink 1.9 executable (required for .bed files)",
    )
    parser.add_argument(
        "--plink2-exe",
        default=None,
        help="Plink 2 executable (required for .pgen files)",
    )

    parser.add_argument(
        "--extract",
        default=None,
        help="A text file with rsids of SNPs to use (one per line)",
    )
    parser.add_argument(
        "--keep",
        default=None,
        help="A text file with ids of individuals to use (two columns per line, each containing FID,IID)",
    )
    parser.add_argument(
        "--num-jk", type=int, default=200, help="number of genomic jackknife blocks"
    )

    parser.add_argument(
        "--memory", type=int, default=2, help="Maximum memory usage (in GB)"
    )
    parser.add_argument("--threads", type=int, default=1, help="Number of CPU threads")

    parser.add_argument(
        "files", nargs="*", help="plink files (either bed files or .pgen files)"
    )
