...
def download_ukb_ld_file(
    chr_num, region_start, overwrite=False, ld_dir=None, no_cache=False
):
...
def compute_ldscores_region(
    df_R_region, df_annot_region, n, is_binary, chr_num, region_start, region_end
):
    ...
    if len(index_intersect) == 0:
        raise ValueError(
            "no SNPs in chromosome %d BP %d-%d had LD info"
            % (chr_num, region_start, region_end)
        )
    if len(index_intersect) < df_R_region.shape[0]:
        logging.warning(
            "Only %d/%d SNPs in chromosome %d BP %d-%d have annotations info. This may severely down-bias the LD-scores"
            % (
                len(index_intersect),
                df_R_region.shape[0],
                chr_num,
    ...
    if df_annot_region.shape[0] != df_R_region.shape[0] or np.any(
        df_annot_region.index != df_R_region.index
    ):
    ...
    df_ldscores_region = pd.DataFrame(
        ld_scores_region,
        index=df_annot_raw_region.index,
        columns=df_annot_raw_region.columns,
    )
    df_ldscores_region = pd.concat(
        (df_annot_region[META_COLUMNS], df_ldscores_region), axis=1
    )
    df_ldscores_region["distance_from_center"] = np.abs(
        df_ldscores_region["BP"] - ((region_start + region_end) // 2)
    )

    return df_ldscores_region


def compute_ldscores_chr(
    df_annot_chr, ld_dir=None, use_ukb=False, n=None, ld_files=None, no_cache=False
):
    ...
    for r in LONG_RANGE_LD_REGIONS:
        ...
        if not np.any(is_in_r):
            continue
        logging.warning(
            "Removing %d SNPs from long-range LD region on chromosome %d BP %d-%d"
            % (is_in_r.sum(), r["chr"], r["start"], r["end"])
        )
        df_annot_chr = df_annot_chr.loc[~is_in_r]
    if df_annot_chr.shape[0] == 0:
        raise ValueError(
            "No SNPs found in chromosome %d (ater removing long-range LD regions)"
            % (chr_num)
        )
    ...
    if np.all(df_annot_chr_raw.dtypes == np.bool):
        ...
        [len(np.unique(df_annot_chr_raw[c])) <= 2 for c in df_annot_chr_raw.columns]
    ):
        is_binary = True
    else:
        is_binary = False
    ...
    if use_ukb:
        for region_start in tqdm(range(1, df_annot_chr["BP"].max() + 1, REGION_LENGTH)):
            ...
            region_end = region_start + REGION_LENGTH
            df_annot_region = df_annot_chr.query(
                "%d <= BP <= %d" % (region_start, region_end)
            )
            if df_annot_region.shape[0] == 0:
                continue

            # download the LD data
            df_R_region = download_ukb_ld_file(
                chr_num, region_start, ld_dir=ld_dir, no_cache=no_cache
            )

            df_ldscores_region = compute_ldscores_region(
                df_R_region,
                df_annot_region,
                n=UKB_N,
                is_binary=is_binary,
    ...
    df_ldscores_chr = df_ldscores_chr.loc[
        ~df_ldscores_chr.index.duplicated(keep="first")
    ]
...
def main(args):
    ...
    if args.gz_out:
        df_ldscores.to_csv(
            args.out, sep="\t", compression="gzip", index=False, float_format="%0.3f"
        )
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="if specified, the script will download and use UK Biobank LD files",
    )
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="if specified, the output file will be a gzipped text file instead of a parquet file",
    )
    parser.add_argument(
        "--ld-dir",
        default=None,
        help="the path of an LD files directory. If not provided, LD files will be downloaded to a temporary directory",
    )
    parser.add_argument(
        "--no-cache",
        default=False,
        action="store_true",
        help="If this flag is specified, the LD files will be removed from the ld-dir after downloading them to save disk space",
    )
    ...
    if args.ukb:
        ...
        if len(args.files) == 0:
            raise ValueError(
                "no input LD files specified. Did you mean to provide the flag --ukb?"
            )
